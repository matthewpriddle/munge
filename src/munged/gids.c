 ...
 *  ghost_hash thereby allowing the event to be logged should the user
...
struct gids {
    ...
    hash_t              gid_hash;       /* hash of supplementary groups      */
...
static void _gids_gid_node_dump (gid_head_p g, uid_t *uidp, void *null);
static void _gids_uid_hash_dump (hash_t uid_hash);
static void _gids_uid_node_dump (uid_node_p u, char *user, void *null);
static void _gids_ghost_hash_dump (hash_t ghost_hash);
static void _gids_ghost_node_dump (char *data, char *user, void *null);
#endif /* _GIDS_DEBUG */
 ...
 */
    ...
    if (!user) {
        ...
        log_msg (LOG_WARNING, "Failed to insert \"%s\" into ghost_hash", user);
        free (p);
    }
    else {
        return (0);
...
static void
_gids_gid_node_dump (gid_head_p g, uid_t *uidp, void *null)
{
    gid_node_p node;
...
static void
_gids_uid_node_dump (uid_node_p u, char *user, void *null)
{
    assert (u->user == user);
...
_gids_ghost_hash_dump (hash_t ghost_hash)
{
    ...
    if (n < 0) {
        log_err (EMUNGE_SNAFU, LOG_ERR,
                "Failed _gids_host_hash_dump(): invalid ghost_hash ptr");
    }
...
static void
_gids_ghost_node_dump (char *data, char *user, void *null)
{
    assert (data == user);
